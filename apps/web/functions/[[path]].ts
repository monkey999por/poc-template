/**
 * Catch-all Pages Function for Remix SSR
 * このファイルが全てのルートをキャッチしてRemixのSSRを処理します
 */
...
export const onRequest = async (context: EventContext<any, any, any>) => {
  ...
  try {
    ...
    if (typeof (build as any).handleRequest === 'function') {
      return await (build as any).handleRequest(context.request, context.env, context);
    }
